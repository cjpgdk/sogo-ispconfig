...
class UpdateClass10 extends PHPUpdateBaseClass {
    ...
    public function run() {
        ...
        $del_files = array(
            $ispchome . '/interface/web/admin/lib/lang/en_sogo_module_bulk_update_config.lng'
        );

        echo PHP_EOL;
        echo "Some file have bean removed! i need your comfimation before deleting them" . PHP_EOL;
        echo "Delete the files? [y/n] (y)";
        if (strtolower(Installer::readInput('y')) == 'y') {
            foreach ($del_files as $value) {
                echo PHP_EOL . "{$value}" . PHP_EOL;
                echo PHP_EOL . "Delete: [y/n] (y)";
                if (strtolower(Installer::readInput('y')) == 'y') {
                    @unlink($value);
                }
                echo PHP_EOL;
